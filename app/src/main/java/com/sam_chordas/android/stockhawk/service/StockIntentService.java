...
public class StockIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        Log.d(StockIntentService.class.getSimpleName(), getString(R.string.stock_intent_service_string));
        ...
        if (stockTaskService.onRunTask(new TaskParams(intent.getStringExtra(getResources().getString(R.string.string_tag)), args)) == GcmNetworkManager.RESULT_FAILURE) {
                ...
                @Override
                public void run() {
                    Toast.makeText(getApplicationContext(), R.string.symbol_add_error, Toast.LENGTH_LONG).show();
                }
            });
        } else {
                handler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(getApplicationContext(), R.string.symbol_add_success, Toast.LENGTH_LONG).show();
                }
            });
        }
    }
